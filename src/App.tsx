...
import LaboralHistoryView from "./components/LaboralHistory";
...
function App() {
  ...
  const buildComponent = (element: React.ComponentType<{lang: string; theme: string}>) => {
    return (
      <div style={{ zIndex: 1 }}>
        {React.createElement(
          element, {
    ...
    <div className="portfolio-main-body">
      ...
      <main className="container portfolio-container" style={{ position: 'relative' }}>
        ...
        <div id={config.ANIMATION_CONTAINER_ID} style={{
          ...
          <canvas
            className='portfolio-animation-canvas'
            ref={canvasRef}
          />
        </div>
